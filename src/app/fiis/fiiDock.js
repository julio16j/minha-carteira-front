...
export default function FiiDock ({submitCallback, initialValues={}, updateValue = false, setUpdateFalse}) {
    const {
        register,
        handleSubmit,
        reset,
            ...
            <form onSubmit={handleSubmit(submitCallback)} className="flex pb-4 justify-center items-center flex-col gap-4">
                <div className="flex gap-4 flex-wrap justify-center flex-row">
                    <div className="flex flex-col max-w-[13rem]">
                        <Input readOnly label="Ticker" variant="bordered" {...register('ticker')} placeholder="Insira o ticker do fii" />
                        {errors.ticker && (
